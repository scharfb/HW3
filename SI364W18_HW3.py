...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgres://Bradley_Scharf@localhost/scharfbHW3"
## Provided:
...
class TweetForm(FlaskForm):
    text = StringField("Enter the text of the tweet (no more than 280 chars):", validators=[Required(), Length(min=0, max=280)])
    username = StringField("Enter the username of the twitter user (no @!):", validators=[Required(), Length(min=0, max=64)])
...
def index():
    ...
    num_of_tweets = len(Tweet.query.all())
    ...
    ## And redirect to the list of all all_tweets
    ...
    return render_template('index.html', form = form, num_tweets = num_of_tweets) # TODO 364: Add more arguments to the render_template invocation to send data to index.html
...
def get_longest_tweet():
    ...
    length = 0
    for tweet in tweets:
        temp_tweet = tweet.text
        length_temp_tweet_nw = len(temp_tweet.replace(" ", ""))
        if length_temp_tweet_nw > length:
            longest_tweet = tweet 
            length = length_temp_tweet_nw
            user = User.query.filter_by(id=longest_tweet.user_id).first()
    return render_template("longest_tweet.html", longest_tweet=longest_tweet.text, user=user.username, display_name=user.display_name)
